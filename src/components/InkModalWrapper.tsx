...
export default function InkModalWrapper({ open, onClose, frames, onChoose, statsVars, inkStatCheck }: InkModalWrapperProps) {
  return (
    <Suspense fallback={<div style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>Loading...</div>}>
      <InkModal open={open} onClose={onClose} frames={frames} onChoose={onChoose} statsVars={statsVars} inkStatCheck={inkStatCheck} />
    </Suspense>
  )
}
