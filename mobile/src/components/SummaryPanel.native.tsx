...
import { PLAYER_VITAL_KEYS, SKILL_COLORS, VITAL_DEFINITIONS, type VitalKey } from "@shared/utils/ui.native"
...
const SmallVerticalBar = ({ value, max = 10, segments = 10, height = MAIN_BAR_HEIGHT, width = 40, color = "#4f82ff" }: { value?: number; max?: number; segments?: number; height?: number; width?: number; color?: string }) => {
  const safeMax = Math.max(1, max)
  const clamped = Math.max(0, Math.min(safeMax, value ?? 0))
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.ageLabel}>{ageYears} yrs</Text>
              ...
              <View key={vital.key} style={styles.vitalRow}>
                <View style={styles.vitalTopRow}>
                  <View style={styles.vitalIconWrapper}>
                    <Feather name={iconName} size={16} color={neutral} />
        ...
        <Text style={styles.occupationText}>{titleText}</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  vitalsList: { marginTop: 12 },
  vitalRow: { marginBottom: 1 },
  ...
  vitalValue: { color: "#9fa3b5", fontWeight: "600", marginLeft: 6, fontSize: 12, width: 25, textAlign: "left" },
  ...
  skillLabel: { marginTop: 6, fontSize: 12, color: "#9fa3b5" },
